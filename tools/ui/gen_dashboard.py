...
function render(D) {
  ...
  drawEquity(D.equity||[], D.paper_equity||[]);
...
function drawLine(ctx,vals,pl,pt,pw,ph,xS,lo,hi,color,label,dash){
  ctx.beginPath();
...
function drawEquity(data, paper) {
  ...
  var pvals=(paper&&paper.length)?paper.map(function(x){return x.equity;}):[];
  var bh=data.map(function(x){return x.benchmark_equity;}).filter(function(v){return v!=null;});
  var lo=Math.min.apply(null,vals),hi=Math.max.apply(null,vals);
  if(pvals.length){lo=Math.min(lo,Math.min.apply(null,pvals));hi=Math.max(hi,Math.max.apply(null,pvals));}
  ...
  var n=Math.max(vals.length,pvals.length);
  var xS=pw/(n-1||1);
  if(bh.length) drawLine(ctx,bh,pl,pt,pw,ph,xS,lo,hi,'rgba(139,148,158,0.5)','持有');
  drawLine(ctx,vals,pl,pt,pw,ph,xS,lo,hi,'#58a6ff','回测');
  if(pvals.length) drawLine(ctx,pvals,pl,pt,pw,ph,xS,lo,hi,'#f59e0b','纸面盘',[6,4]);
